...
import numpy as np
def solution(m):
    #Absorbing Markov Chain problem
    mat = np.array(m)
    rowsum = np.sum(mat,axis=1)
    colsum = np.sum(mat,axis=0)
    
    terminal_states = [i for i in range(len(rowsum)) if rowsum[i]==0]
    
    #Shifting the columns first
    temp_col = mat[:,terminal_states]
    reduce_mat = np.delete(mat,terminal_states,axis=1)
    temp_mat = np.append(temp_col,reduce_mat,axis=1)
    #Shiftimg the rows now
    temp_row = temp_mat[terminal_states,:]
    reduce_mat = np.delete(temp_mat,terminal_states,axis=0)
    final_mat = np.append(temp_row,reduce_mat,axis=0)
'''
